...
class PointTests(APITester):
    ...
    def test_post_point_no_tags(self):
        ...
        added_point_instance = session.query(Point).get(added_point['point_id'])
        self.assertEqual(added_point['question'], 'what is sah?')
        self.assertEqual(added_point['answer'], 'sah')


    def test_post_point_with_tags(self):
        session = self.session
        a_tag = Tag(name='a')
        b_tag = Tag(name='b')
        c_tag = Tag(name='c')
